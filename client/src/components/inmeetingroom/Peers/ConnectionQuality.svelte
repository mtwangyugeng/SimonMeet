<script context=module>
    ...
    const subConnectionScoreByPeerId = (peerId, setScore) => hmsStore.subscribe((score) => {
        setScore(score?.downlinkQuality);
...
<script>
    ...
    const unsub = subConnectionScoreByPeerId(peerId, (score) => networkScore = score);
    
    onDestroy(unsub);
</script>
<section>
  {#if (networkScore !== undefined && networkScore > 0)}
        <!-- taken from: https://github.com/100mslive/svelte-100ms/commit/6796c79e61d7fe14c4689790337c00b69cb61209 -->
        <svg
            viewBox="0 0 14 12"
